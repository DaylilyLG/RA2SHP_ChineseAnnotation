...
- 本文使用了 Markdown 标记语言来优化阅读体验，所以我们首先需要准备一个代码编辑器（作者使用的是 [HBuilderX](https://www.dcloud.io/hbuilderx.html)，如果你使用 [VSCode](https://code.visualstudio.com/) 可能需要手动下载 Markdown 插件），或者用电脑自带的记事本，但不推荐（因为修改过程需要一个方便查看代码的环境），安装代码编辑器后就可以用它来打开这个介绍文件了，打开后可以看到带有 “#”（井号）标题的前方有折叠按键，单击即可折叠/展开，看完了就能折叠隐藏起来，非常方便阅读。还可以到[我的个人网站](https://daylily.website/)在线观看哦（还在施工中）  
...
打开搜索（按 Ctrl + F 进行搜索），搜索 [AEGIS]，括号内的 AEGIS 指的是神盾巡洋舰，为确保搜索到正确内容，注意要带上`[]`括号，你会看到这样的代码：  
~~~ini
[AEGIS]  
UIName=Name:AEGIS  
Name=Aegis Cruiser  
...
~~~
没错，这一部分就是神盾巡洋舰的**单位本体**（特指这部分的代码），还可以简称**单位**（泛指所有相关的代码）。其中 AEGIS 是神盾巡洋舰的**注册名**，如果将 AEGIS 带上括号“[AEGIS]”，那么你可以称它为**节**（规范性称法），而在 [AEGIS] 单位本体**节**的下方代码可以被称为**语句**或**标签**，示意代码：  
~~~ini
[AEGIS]     节/(神盾巡洋舰的注册名)  
UIName=     语句/标签/代码  
Name=       语句/标签/代码 
......=     语句/标签/代码  
~~~
可以说是某个**标签**，或者某个**语句**，甚至统称为**代码**也是没问题的，请依据个人习惯来决定，而每个**节**下方携带的各种代码，你可以看做*一个整体或相关部分*。  
仔细看看，还会发现有的语句或节后边会带有 “;” 这个符号，这表示*在这符号之后的一行代码将无效化*，即成为**注释**，我们可以直接无视它们，例如控制单位建造数量的是这个语句：`;BuildLimit=2`，可以看到已经被注释了，所以不会有任何作用，忽略即可。我们也可以手动取消注释（把 “;” 这个符号删除），或把注释保留下来，另起一行，添加语句`BuildLimit=`，例如：  
~~~ini
[AEGIS]
......省略一大段......  
...
在 [AEGIS] 单位本体节的下方可以看到这个语句`Primary=Medusa`，我们可以把`Primary`和`Medusa`分成两部分来看，`Primary`可以看作是**武器类型**，`Medusa`则是**武器**（不难看出，这行代码给单位指定了 [Medusa] 作为它的武器），下面先来认识一下**武器类型**：  

- 主武器`Primary=`：单位的主要武器，以及单位的三星主武器`ElitePrimary=`，单位升级至三星后会立即换用`ElitePrimary=`所指定的武器。需要注意的是，有的单位会出现没有武器类型语句或相关武器类型语句被注释的情况，例如只有主武器的遥控坦克、工程师等，即便升级到三星，也会因为没有`ElitePrimary=`语句，而继续使用`Primary=`所指定的武器。出现没有使用任何武器相关代码的情况一般常见于平民单位（没有主(副)武器和三星主(副)武器等武器类型）。
...
**全局语句**（或全局标签）在规则文件中一般是指`[General]`全局节下方的所有语句代码，可以理解为*独立存在的代码*，例如控制各个阵营的步兵类型（例如盟军玩家建造的建筑被摧毁时逃出来的就是美国大兵）、幻影坦克可以伪装的物体、间谍可以偷多少比例的钱等等，你甚至可以让“维修小扳手”自动维修建筑，而无需手动操作。这部分有兴趣可以查阅词典，新手只需粗略了解即可。  
  
现在我们来具体了解一下什么是**单位**，单位的类型有很多，例如：车辆、步兵、建筑、海军，还有空军等等都可以被称作**单位**。其中，车辆、直升机、飞艇、飞碟及海军被分类为***载具*** ← 重点，具体分类会在之后介绍。  
  
了解载具前，我们先来了解一下什么是*注册表*:
  
上面有提到过注册名，例如`AEGIS`是神盾巡洋舰的**注册名**，
你可能会有疑问，为啥叫**注册名**？它是咋来的？现在来搜索这个`[VehicleTypes]`节，你会看到：  
...
Lifetime=30;设置喷火武器用于产生粒子的时间（例如数值越高，会使喷火武器单次攻击的持续时间越长，期间将持续释放粒子，间接影响一次性产生的粒子数量）
...
首先我们要知道，在单位本体中指定单位名字的语句是：`UIName=Name:E1`（美国大兵），显然，我们只需在该文件中搜索`[Name:E1]`节即可，你会发现`Value=美国大兵`，那么我们只需将`美国大兵`修改成你喜欢的名字就好~ 
...
①、神盾巡洋舰这个单位本体对应的是`[AEGIS]`，搜索`[AEGIS]`。记得带上括号（废话）
...
# 第五章：地图文件和地图编辑器
  ...
  同样，地图文件中也有大量与地图相关的*语句*内容，不过我们通常只用*地图编辑器*来编辑它们。下面我们来了解一下*地图编辑器*（以下简称地编），我们若要修改地图（例如地形等），就要用到地编来打开这些地图文件：  
...
※准备放在正文的注意事项：
  以HDM地编1.3.5版本为例，该版本支持保存地图文件中的注释，要注意该功能并非默认开启，需要在“选择”->“偏好设置”中开启注释保存功能（注：这将使地编的版本号注释、编码警告注释等信息保存功能无效化）
  当地图保存后，地图文件内的节和代码默认会重新排序，例如手动加入地图文件中的武器、单位本体等等部分，不影响游戏运行。可在“选择”->“偏好设置”中，开启“维持已有的ini小节顺序”功能，开启后，除了地图相关的节或语句代码，将不会改变其他节的顺序，但无论是否开启该功能，始终默认每个节下方的代码会依据语句或标签长短，从上到下，从短到长排序。默认添加地编信息注释。
  
  ※这里准备介绍Rampastring的 [WorldAlteringEditor](https://github.com/CnCNet/WorldAlteringEditor/releases)  （以下简称WAE）  
  
※准备放在正文的注意事项：
  以WAE地编1.7.8版本为例，WAE不支持保存注释，默认保存地图后自动清除注释
  当地图保存后，地图文件内的节默认会重新排序，不影响游戏运行。默认添加地编信息注释
