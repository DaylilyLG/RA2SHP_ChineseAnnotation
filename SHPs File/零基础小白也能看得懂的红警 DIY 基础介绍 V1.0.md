...
红色警戒2（简称 RA2）、尤里的复仇（简称 YR），由于游戏源码未公开，在大家 DIY（修改游戏数据或自定义内容）的时候或多或少会遇到游戏引擎中的硬编码限制。因此，许多红警爱好者在基于 YR 1.001 版本的基础上，开发了新的扩展引擎平台，来修复或扩充原版引擎的功能，这极大丰富了 DIY 的自由度，例如：Aers、Phobos、Kratos等。但考虑到许多新人并没有接触过这些引擎，所以本次将不使用扩展引擎的功能进行介绍，以尽量贴合新人从 “0” 开始接触 DIY 的情景，但还是建议新人预装 Aers 和 Phobos 这两个引擎，可以改善游戏体验，例如提升原版引擎的启动速度。
...
全局标签在规则文件中一般是指`[General]`标签下方的所有语句代码，例如控制困难、中等、简单的AI行为，以及各个阵营的建筑会出来哪些步兵（例如盟军建筑被摧毁时逃出来的是美国大兵、苏军是动员兵），等等。
  
※注册表及注册内容  
  
上面有提到过注册名，例如`[AEGIS]`是神盾巡洋舰的*注册名*，现在来细讲一下什么是注册（待完善）  
...
其他一些手法：":"冒号[TEST2]:[TEST] 就是复制[TEST2]的所有代码的前提下，再往[TEST2]中加入新代码或语句，意为“继承”（不知道要不要放在介绍内容里细讲...先预留）
...
DIY 新手需要善用搜索功能，在说明书或 [DIYRA2在线词典](https://wiki.ra2diy.com/w/Category:%E8%AF%8D%E5%85%B8) 中（Ctrl + F）自行查询标签和各种代码的功能和含义，简单又高效~  
...
巴拉巴拉（添加新单位打算放这里讲，不过作者更新速度非常慢...等啥时候有大把空闲时间再来做这个...B站还有许多UP主在视频中比我讲的还好又专业，可以去看看，像我这种纯文字的表述不一定适合所有人...）  
...
# 第五章：地图文件和地图编辑器
  ...
  下面我们来了解一下*地图编辑器*（以下简称地编），我们若要修改地图，就要用到地编来打开这些地图文件：  
