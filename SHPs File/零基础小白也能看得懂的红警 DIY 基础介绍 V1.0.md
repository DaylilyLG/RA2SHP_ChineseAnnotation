...
※准备放在正文的注意事项：
  ...
  当地图保存后，地图文件内的代码默认会重新排序，例如手动加入地图文件中的武器、单位本体等等部分，但不影响运行。（可在“选择”->“偏好设置”中，开启“维持已有的ini小节顺序”功能，开启后，除了地图相关的语句代码，将不会改变其他代码的顺序）默认添加地编信息注释，会根据语句长短从短到长排序。
...
※准备放在正文的注意事项：
  ...
  当地图保存后，地图文件内的代码默认会重新排序，例如手动加入地图文件中的武器、单位本体等等部分，但不影响运行。默认添加地编信息注释
