...
**全局语句**（或全局标签）在规则文件中一般是指`[General]`标签下方的所有语句代码，可以理解为*独立存在的代码*，例如控制各个阵营的步兵类型（例如盟军玩家建造的建筑被摧毁时逃出来的就是美国大兵）、幻影坦克可以伪装的物体、间谍可以偷多少比例的钱等等，你甚至可以让“维修小扳手”自动维修建筑，而无需手动操作。这部分有兴趣可以查阅词典，新手只需粗略了解即可。  
...
※准备放在正文的注意事项：
  以HDM地编1.3.5版本为例，该版本支持保存地图文件中的注释，要注意该功能并非默认开启，需要在“选择”->“偏好设置”中开启注释保存功能（注：这将使地编的版本、编码警告注释等信息保存功能无效化）
  当地图保存后，地图文件内的代码默认会重新排序，例如手动加入地图文件中的武器、单位本体等等部分，但不影响运行。（可在“选择”->“偏好设置”中，开启“维持已有的ini小节顺序”功能，开启后，除了地图相关的语句代码，将不会改变其他代码的顺序）默认添加地编信息注释
