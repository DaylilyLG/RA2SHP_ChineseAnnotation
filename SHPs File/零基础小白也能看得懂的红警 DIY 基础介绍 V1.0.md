...
```
对于新手来说或许不能很好地理解粒子系统的概念，其实在游戏中，我们到处都能“看见”*粒子*，你可以把粒子看作一个隐形的像素点，粒子通常会携带一个居中显示的图像，比如坦克受损时会产生一团团的烟雾，这是因为粒子在向上运动，每一团烟雾代表一个粒子，所以你能观察到不断向上冒出的烟雾。我们在游戏里看见病毒狙击手射杀一名步兵时，会产生一团团绿色的毒气，这些毒气到处飘动，也是因为这些*粒子*在运动。  
...
UseFireParticles=yes;使弹头动画不播放、武器本体中的伤害Damage无效（允许粒子伤害Damage）、粒子动画Image挂弹头的伤害Damage无效，可使用 Ares 修复（无法修复粒子动画挂弹头的伤害）
...
烟雾（载具受损的烟雾效果），火花（维修车修载具时喷溅的火花），毒气（病毒狙击手击杀步兵后产生的毒气）是最常见的类型，当你看到以上语句，说明该单位或武器启用了粒子系统。  
